...
def grade_all(assignment_module, logger, verbose=False):
    ...
    logger.info(
        f"Total                                                    {score:3d} / {total_score:3d}"
    )
    ...
    if assignment_path.is_dir():
        ...
        with zipfile.ZipFile(assignment_path) as f:
            ...
            if len(module_names) != 1:
                logger.error(
                    f"Malformed zip file, expected one top-level folder, got {len(module_names)}"
                )
                return None
...
class RuntimeFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # color full line
        output_lines = [
            colored(line, self._COLOR[record.levelname]) for line in output.splitlines()
        ]
        # add prefix
...
def init_loggers(log_path: str, show_debug=False, disable_color=False):
    formatter = RuntimeFormatter(
        "[%(levelname)-8s %(elapsed)s] %(message)s", disable_color=disable_color
    )
...
def run():
    ...
    logger = init_loggers(
        args.log_path, show_debug=args.very_verbose, disable_color=args.disable_color
    )
